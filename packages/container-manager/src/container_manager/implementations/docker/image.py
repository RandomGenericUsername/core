...
class DockerImageManager(ImageManager):
    ...
    def build(
        ...
        if isinstance(context.dockerfile, Path):
            # Mode 1: Dockerfile is a path
            dockerfile_path = context.dockerfile
            context_dir = context.context_path or dockerfile_path.parent
        ...
        elif context.context_path:
            # Mode 2: Dockerfile is string, but explicit context path provided
            context_dir = context.context_path
            dockerfile_path = context_dir / "Dockerfile"
            dockerfile_path.write_text(context.dockerfile, encoding="utf-8")
            ...
            # Mode 3: Dockerfile is string, no context path -> Tarball to stdin
