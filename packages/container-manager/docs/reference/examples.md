...
```python
from container_manager import BuildContext
...
```python
from container_manager import RunConfig
...
```python
from container_manager import RunConfig, VolumeMount, PortMapping
...
```python
import time
from container_manager import ContainerEngineFactory, RunConfig
...
```python
from container_manager import ContainerEngineFactory, BuildContext
import logging
...
### Example 9: Build from Dockerfile Path
...
# Method 1: Path to Dockerfile (context defaults to parent dir)
context_path = BuildContext(
    dockerfile=Path("./Dockerfile")
)
image_id = engine.images.build(context_path, "my-image:path")

# Method 2: Path to Containerfile (Podman style)
context_podman = BuildContext(
    dockerfile=Path("./Containerfile")
)
image_id_podman = engine.images.build(context_podman, "my-image:podman")

# Method 3: Explicit Context Directory
context_explicit = BuildContext(
    dockerfile=Path("./docker/Dockerfile.prod"),
    context_path=Path(".")  # Use root as context
)
image_id_explicit = engine.images.build(context_explicit, "my-image:prod")
```
