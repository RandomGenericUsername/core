...
```bash
# Add as a dependency
uv add container-manager

# Or add to pyproject.toml
[dependencies]
container-manager = { path = "src/common/modules/container_manager" }
```

### Verify Installation

```python
from container_manager import ContainerEngineFactory, ContainerRuntime
...
```python
from container_manager import RunConfig, VolumeMount
...
```python
from container_manager import RunConfig, PortMapping
...
3. **Handle exceptions:**
   ```python
   from container_manager import ImageBuildError
